...
void UART_Configuration(uint16_t baudrate){ 
  ...
  UART1->CR2 &= ~(1 << 2);  // Disable UART Receiver  
UART1->CR2 |= (1 << 2);   // Enable UART Receiver again 
...
INTERRUPT_HANDLER(UART1_RX_IRQHandler,18){
  if (UART1_GetFlagStatus(UART1_FLAG_RXNE) && receive_flag == 1) {  // Check if data is available
