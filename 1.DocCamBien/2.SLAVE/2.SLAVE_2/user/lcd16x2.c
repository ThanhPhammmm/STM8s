...
void Send_RTC_To_LCD(uint8_t *data_sensor3) {
  ...
  LCD_Set_Cursor(1, 1);
  LCD_SendString(days_of_week[data_sensor3[0]]); // Print day-of-week
  LCD_SendData('/');
  LCD_Send16BitNumber_RTC(data_sensor3[2],2);                      // Print day number
  LCD_SendData('/');
  LCD_SendString(months[data_sensor3[1] - 1]);        // Print month
  LCD_SendData('/');
  LCD_Send16BitNumber_RTC((2000 + data_sensor3[3]) / 100,2);               // Print century (20)
  LCD_Send16BitNumber_RTC((2000 + data_sensor3[3]) % 100,2);               // Print year (04)

  // Print the second line: "07:00:00"
  LCD_Set_Cursor(2, 1);
  LCD_Send16BitNumber_RTC(data_sensor3[4],2);
  LCD_SendData(':');
  LCD_Send16BitNumber_RTC(data_sensor3[5],2);
  LCD_SendData(':');
  LCD_Send16BitNumber_RTC(data_sensor3[6],2);
}