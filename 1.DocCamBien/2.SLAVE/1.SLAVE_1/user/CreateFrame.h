...
typedef enum
{
    TYPE_MESSAGE_SET_SLAVE            = 0x0001,
    TYPE_MESSAGE_CONFIRM_SLAVE        = 0x0002,
...
#define LENGTH_MESSAGE_SET_SLAVE     5      /* Length_data + Port + Checksum*/
...
uint8_t Message_Frame_Create_Confirm_Slave(uint8_t datain_port, uint8_t datain_sensor, uint8_t* dataout);
...
uint8_t Message_Frame_Response_RES(slave_e datain_port, type_Sensor_e datain_sensor, uint16_t datain_value, uint8_t* dataout);
