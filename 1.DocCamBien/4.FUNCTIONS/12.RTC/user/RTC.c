...
while (I2C->SR3 & I2C_SR3_BUSY);  // Ch? khi bus b?n
    ...
    // Send DS1307 address with Write mode (0xD0) to set register pointer
    I2C_Send7bitAddress(DS1307_ADDR, I2C_DIRECTION_TX);
    while (!I2C_CheckEvent(I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED));
    ...
    // Send DS1307 address with Read mode (0xD1)
    I2C_Send7bitAddress(DS1307_ADDR, I2C_DIRECTION_RX);
    while (!I2C_CheckEvent(I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED));

    // Read data and send NACK to indicate end of reception
...
void Get_Current_Time(RTC_Time_t *RTC_Time){
    ...
    RTC_Time->minutes = Bcd_To_Binary(min);
